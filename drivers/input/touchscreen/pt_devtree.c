...
{
	return of_property_read_string(dev_node, "pt,inp_dev_name",
			inp_dev_name);
}
		...
		struct device_node *dev_node)
{
	...
	abs = create_and_get_u16_array(dev_node, "pt,abs", &size);
 ...
 *  information for all vitual keys
 *
		...
		const char *inp_dev_name, struct pt_virtual_keys *vkeys)
{
	...
	data = create_and_get_u16_array(dev_node, "pt,virtual_keys", &size);
	if (data == NULL)
		return 0;
	else if (IS_ERR(data)) {
		rc = PTR_ERR(data);
...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	/* Optional fields */
	rc = of_property_read_u32(dev_node, "pt,flags", &value);
	if (!rc)
		pdata->flags = value;

	rc = of_property_read_u32(dev_node, "pt,vkeys_x", &value);
	if (!rc)
		pdata->vkeys_x = value;

	rc = of_property_read_u32(dev_node, "pt,vkeys_y", &value);
	if (!rc)
		pdata->vkeys_y = value;
 ...
 *
 * SUMMARY: Free all pointers that include touch framework, vitual keys and
 *  touch data.
 *
 * PARAMETERS:
 *   *vkeys  - pointer to virtual key structure
...
static char *touch_setting_names[PT_IC_GRPNUM_NUM] = {
	...
	"pt,cmd_regs",		/* PT_IC_GRPNUM_CMD_REGS */
	"pt,tch_rep",		/* PT_IC_GRPNUM_TCH_REP */
	"pt,data_rec",		/* PT_IC_GRPNUM_DATA_REC */
	"pt,test_rec",		/* PT_IC_GRPNUM_TEST_REC */
	"pt,pcfg_rec",		/* PT_IC_GRPNUM_PCFG_REC */
	"pt,tch_parm_val",	/* PT_IC_GRPNUM_TCH_PARM_VAL */
	"pt,tch_parm_size",	/* PT_IC_GRPNUM_TCH_PARM_SIZE */
	NULL,			/* PT_IC_GRPNUM_RESERVED1 */
	NULL,			/* PT_IC_GRPNUM_RESERVED2 */
	"pt,opcfg_rec",		/* PT_IC_GRPNUM_OPCFG_REC */
	"pt,ddata_rec",		/* PT_IC_GRPNUM_DDATA_REC */
	"pt,mdata_rec",		/* PT_IC_GRPNUM_MDATA_REC */
	"pt,test_regs",		/* PT_IC_GRPNUM_TEST_REGS */
	"pt,btn_keys",		/* PT_IC_GRPNUM_BTN_KEYS */
		...
		struct device_node *core_node)
{
	...
	/* Required fields */
	rc = of_property_read_u32(core_node, "pt,irq_gpio", &value);
	if (rc)
		goto fail_free;
	pdata->irq_gpio = value;

	rc = of_property_read_u32(core_node, "pt,hid_desc_register", &value);
	if (rc)
		goto fail_free;
	 ...
	 */
	rc = of_property_read_u32(core_node, "pt,rst_gpio", &value);
	if (!rc)
		pdata->rst_gpio = value;

	rc = of_property_read_u32(core_node, "pt,runfw_gpio", &value);
	if (!rc)
		pdata->runfw_gpio = value;

	rc = of_property_read_u32(core_node, "pt,level_irq_udelay", &value);
	if (!rc)
		pdata->level_irq_udelay = value;

	rc = of_property_read_u32(core_node, "pt,vendor_id", &value);
	if (!rc)
		pdata->vendor_id = value;

	rc = of_property_read_u32(core_node, "pt,product_id", &value);
	if (!rc)
		pdata->product_id = value;

	rc = of_property_read_u32(core_node, "pt,flags", &value);
	if (!rc)
		pdata->flags = value;

	rc = of_property_read_u32(core_node, "pt,easy_wakeup_gesture", &value);
	if (!rc)
