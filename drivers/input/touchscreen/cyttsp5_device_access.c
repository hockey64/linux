...
/*******************************************************************************
 * FUNCTION: cyttsp5_response_show
 *
 * SUMMARY: Prints the contents of *buf to kmsg
 *
 * PARAMETERS:
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* write ic_buf to log */
	pt_pr_buf(dev, DL_INFO, dad->ic_buf, length, "ic_buf");
	...
	if (rc) {
		dad->response_length = 0;
		pt_debug(dev, DL_ERROR, "%s: Failed to store command\n",
				__func__);
	} else {
		dad->status = 1;
	}
...
exit:
	mutex_unlock(&dad->sysfs_lock);
	pt_debug(dev, DL_INFO, "%s: return size=%zu\n",
		__func__, size);
	return size;
}
...
start_testing:
	...
	if (!dad->cmcp_force_calibrate) {
		...
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Error calibrating idacs for mutual r=%d\n",
				__func__, rc);
		}
		rc = _cyttsp5_calibrate_idacs_cmd(dev, 1, &status);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Error calibrating idacs for btns r=%d\n",
				__func__, rc);
		}
		rc = _cyttsp5_calibrate_idacs_cmd(dev, 2, &status);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Error calibrating idacs for self r=%d\n",
				__func__, rc);
		}
	}
	/*resume_scanning */
	...
	if (self_test_id_supported)
		pt_debug(dev, DL_ERROR, "cyttsp5_get_cmcp_info failed");
...
static int cyttsp5_suspend_scan_cmd_(struct device *dev)
{
	...
	rc = cmd->nonhid_cmd->suspend_scanning(dev, 0);
	if (rc < 0)
		pt_debug(dev, DL_ERROR, "%s: Suspend scan failed r=%d\n",
			__func__, rc);
	return rc;
}
...
static int cyttsp5_resume_scan_cmd_(struct device *dev)
{
	...
	rc = cmd->nonhid_cmd->resume_scanning(dev, 0);
	if (rc < 0)
		pt_debug(dev, DL_ERROR, "%s: Resume scan failed r=%d\n",
			__func__, rc);
	return rc;
}
...
static int cyttsp5_exec_scan_cmd_(struct device *dev)
{
	...
	if (rc < 0)
		pt_debug(dev, DL_ERROR,
			"%s: Heatmap start scan failed r=%d\n",
			__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *return_buf)
{
	...
	if (rc < 0)
		pt_debug(dev, DL_ERROR,
				"%s: Retrieve scan data failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *data)
{
	...
	if (rc < 0)
		pt_debug(dev, DL_ERROR,
				"%s: Get data structure failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u8 *results_available)
{
	...
	rc = cmd->nonhid_cmd->run_selftest(dev, 0, test_id,
			...
			results_available);
	if (rc < 0)
		pt_debug(dev, DL_ERROR, "%s: Run self test failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *data)
{
	...
	if (rc < 0)
		pt_debug(dev, DL_ERROR,
			"%s: Get self test result failed r=%d\n",
			__func__, rc);
	return rc;
}
		...
		bool get_result_on_pass)
{
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: Error on suspend scan r=%d\n",
				__func__, rc);
		...
		bool get_result_on_pass)
{
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR,
				"%s: Error on request exclusive r=%d\n",
				__func__, rc);
		goto put_pm_runtime;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR,
				"%s: Error on execute panel scan r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR,
				"%s: Error on retrieve panel scan r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
 ...
 *
 * SUMMARY: This function stores the type of panel scan the read method will
 *	perform.
 *
		...
		size_t count, loff_t *ppos)
{
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR,
			"%s: Error on get data structure r=%d\n",
			__func__, rc);
		goto resume_scan;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR,
				"%s: Error on calibrate idacs r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	...
	if (dad->calibrate_initialize_baselines) {
		...
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
					"%s: Error on initialize baselines r=%d\n",
					__func__, rc);
			goto resume_scan;
		}
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	if (rc) {
		pt_debug(dev, DL_ERROR, "%s: Error creating command sysfs\n",
				__func__);
		goto exit;
	}
	...
	if (rc) {
		pt_debug(dev, DL_ERROR,
				"%s: Error, could not create status\n",
				__func__);
	...
	if (rc) {
		pt_debug(dev, DL_ERROR,
				"%s: Error, could not create response\n",
				__func__);
		...
		void *context)
{
	...
	pt_debug(dev, DL_INFO, "%s: Found cmcp threshold file.\n",
		__func__);
