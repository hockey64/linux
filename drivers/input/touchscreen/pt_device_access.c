...
static ssize_t pt_run_and_get_selftest_result_noprint(struct device *dev,
...
/*******************************************************************************
 * FUNCTION: pt_ic_parse_input
 *
 * SUMMARY: Parse any comma or space delimited sysfs user input data into
 ...
 *  *ic_buf       - pointer to ouput buffer
 *   ic_buf_size  - size of output buffer
 ******************************************************************************/
static int pt_ic_parse_input(struct device *dev, const char *buf,
	...
	struct cmcp_data *cmcp_info)
{
	...
	rc = pt_run_and_get_selftest_result_noprint(
	...
	rc = pt_run_and_get_selftest_result_noprint(
		dev, tmp_buf, sizeof(tmp_buf),
		PT_ST_ID_CM_BUTTON, PIP_CMD_MAX_LENGTH, true);
	if (rc) {
	...
	rc = pt_run_and_get_selftest_result_noprint(
		dev, tmp_buf, sizeof(tmp_buf),
		PT_ST_ID_CP_BUTTON, PIP_CMD_MAX_LENGTH, true);
	if (rc) {
...
start_testing:
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: Error on request exclusive rc = %d\n",
			__func__, rc);
	}
	...
	while (retry--) {
		cmd->request_get_scan_state(dev,
				PT_CORE_CMD_UNPROTECTED, &scan_state);
	...
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: Error on release exclusive rc = %d\n",
			__func__, rc);
	}
	pm_runtime_put(dev);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	length = pt_ic_parse_input(dev, buf, size, dad->ic_buf,
			PT_MAX_PRBUF_SIZE);
 ...
 *	*dev - pointer to devices structure
 ******************************************************************************/
static int pt_exec_scan_cmd_(struct device *dev)
{
	int rc;
...
/*******************************************************************************
 * FUNCTION: pt_run_and_get_selftest_result_noprint
 *
 * SUMMARY: Run the selftest and store the test result in the
 *  pt_device_access_data struct. Don't print results to log.
 *
 * RETURN:
 *	 0 = success
 *	!0 = failure
 ...
 * PARAMETERS:
 *  *dev                - pointer to device structure
 *  *buf                - pointer to print buf of return data(invalid)
 *   buf_len            - length of print buf of return data(invalid)
 *   test_id            - selftest id
		...
		size_t count, loff_t *ppos)
{
	...
	cmd->request_get_scan_state(dev, PT_CORE_CMD_UNPROTECTED, &scan_state);
	if (scan_state != PT_SCAN_STATE_INACTIVE) {
		rc = pt_suspend_scan_cmd_(dev);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	length = pt_ic_parse_input(dad->dev, data->pr_buf, count,
			dad->ic_buf, PT_MAX_PRBUF_SIZE);
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	length = pt_ic_parse_input(dad->dev, buf, count, dad->ic_buf,
			PT_MAX_PRBUF_SIZE);
	if (length != 1) {
		pt_debug(dad->dev, DL_ERROR,
				"%s: Malformed input\n", __func__);
		...
		size_t count, loff_t *ppos)
{
	...
	if (!*ppos)
		...
		data->pr_buf_len = pt_run_and_get_selftest_result(
			data->dad->dev, data->pr_buf, sizeof(data->pr_buf),
			PT_ST_ID_OPENS, PIP_CMD_MAX_LENGTH, false);
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	length = pt_ic_parse_input(dev, buf_in, count, dad->ic_buf,
			PT_MAX_PRBUF_SIZE);
	if (length <= 0) {
		pt_debug(dev, DL_ERROR,
